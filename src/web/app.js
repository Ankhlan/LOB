...
(function () {
    ...
    const D = {
        ...
        insLiq24h: $('insLiq24h'),
        insStatus: $('insStatus'),
        insLiqBody: $('insLiqBody'),
        insRefreshBtn: $('insRefreshBtn'),
        insChart: $('insChart'),
    ...
    function toast(msg, type = 'info') {
        // Deduplicate: suppress identical toasts within 5s
        const key = type + ':' + msg;
        const now = Date.now();
        if (_recentToasts[key] && now - _recentToasts[key] < 5000) return;
    ...
    function connectSSE() {
        ...
        es.addEventListener('circuit_breaker', (e) => {
            try { onCircuitBreakerMsg(JSON.parse(e.data)); } catch (err) { console.error('[CRE][SSE] circuit_breaker parse error:', err); }
        });
        es.addEventListener('alert', (e) => {
            try { onAlertMsg(JSON.parse(e.data)); } catch (err) { console.error('[CRE][SSE] alert parse error:', err); }
        });
    ...
    function onPositionsMsg(positions) {
        if (Array.isArray(positions)) S.positions = positions;
        else if (Array.isArray(positions?.positions)) S.positions = positions.positions;
        else if (Array.isArray(positions?.data)) S.positions = positions.data;
        else S.positions = [];
        renderPositions();
    }
    function onOrdersMsg(orders) {
        if (Array.isArray(orders)) S.orders = orders;
        else if (Array.isArray(orders?.orders)) S.orders = orders.orders;
        else if (Array.isArray(orders?.data)) S.orders = orders.data;
        else S.orders = [];
        renderOrders();
    }
    ...
    // Circuit breaker banner handler
    function onCircuitBreakerMsg(data) {
        const { symbol, state, message } = data;
        if (!D.cbBanner || !D.cbBannerText) return;
        if (state === 'NORMAL') {
            D.cbBanner.classList.add('hidden');
            D.cbBanner.className = 'cb-banner hidden';
            return;
        }
        D.cbBanner.className = 'cb-banner';
        if (state === 'LIMIT_UP') D.cbBanner.classList.add('limit-up');
        else if (state === 'LIMIT_DOWN') D.cbBanner.classList.add('limit-down');
        D.cbBannerText.textContent = `TRADING HALTED: ${symbol} — ${message || state}`;
        D.cbBanner.classList.remove('hidden');
        Sound.error && Sound.error();
    }

    // Price alert notification handler
    function onAlertMsg(data) {
        const { symbol, alert_price, current_price, direction, message } = data;
        const dir = direction === 'ABOVE' ? '▲' : '▼';
        const text = message || `${symbol} ${dir} ${fmtMNT(alert_price)} (now ${fmtMNT(current_price)})`;
        toast(`🔔 Price Alert: ${text}`, 'warn');
        Sound.fill && Sound.fill();
    }
    ...
    async function fetchInsuranceFund() {
        try {
            ...
            if (data) {
                S.insurance = { ...S.insurance, ...data, error: null };
                updateInsuranceDisplay(data);
            } else {
                throw new Error('No insurance data returned');
            }
        } catch (err) { 
            console.error('[CRE] fetchInsuranceFund error:', err);
            S.insurance.error = 'Connection lost';
            showInsuranceError('Connection lost');
        }
    }

    function updateInsuranceDisplay(data) {
        if (!data) return;
        
        // Update fund overview cards
        D.insBalance.textContent = data.balance ? fmtMNT(data.balance) : 'Data unavailable';
        
        if (data.change !== undefined && data.change !== null) {
            const change = data.change || 0;
            D.insChange.textContent = (change >= 0 ? '+' : '') + fmtMNT(change);
            D.insChange.className = change >= 0 ? 'positive' : 'negative';
        } else {
            D.insChange.textContent = '—';
            D.insChange.className = '';
        }
        
        D.insRatio.textContent = data.ratio !== undefined ? (data.ratio || 0).toFixed(1) + '%' : '—';
        D.insLiq24h.textContent = data.liquidations24h !== undefined ? (data.liquidations24h || 0).toString() : '—';
        
        D.insStatus.textContent = data.status ? data.status.toUpperCase() : 'UNKNOWN';
        D.insStatus.className = 'ins-card-val status ' + (data.status === 'operational' ? 'green' : 'red');
        
        // Update recent liquidations table
        renderRecentLiquidations(data.recentLiquidations || []);
        
        // Update chart if data available
        if (data.fundHistory && data.fundHistory.length) {
            renderInsuranceChart(data.fundHistory);
        }
    }

    function showInsuranceError(message) {
        // Show error state - no fake data
        D.insBalance.textContent = message;
        D.insChange.textContent = '—';
        D.insChange.className = '';
        D.insRatio.textContent = '—';
        D.insLiq24h.textContent = '—';
        D.insStatus.textContent = 'UNAVAILABLE';
        D.insStatus.className = 'ins-card-val status red';
        
        // Clear liquidations table
        D.insLiqBody.innerHTML = '<tr class="empty-row"><td colspan="7">Data unavailable</td></tr>';
        
        // Clear chart
        if (D.insChart) {
            const ctx = D.insChart.getContext('2d');
            const canvas = D.insChart;
    ...
    function renderRecentLiquidations(liquidations) {
        if (!liquidations.length) {
            D.insLiqBody.innerHTML = '<tr class="empty-row"><td colspan="7">No recent liquidations</td></tr>';
            return;
        }

        D.insLiqBody.innerHTML = liquidations.slice(0, 20).map(liq => {
            const sideClass = liq.side === 'long' || liq.side === 'buy' ? 'green' : 'red';
            const impactClass = (liq.impact || 0) >= 0 ? 'green' : 'red';
            
            return `<tr>
                <td>${timeStr(liq.time)}</td>
                <td class="col-sym">${liq.symbol || '—'}</td>
                <td class="${sideClass}">${(liq.side || '').toUpperCase()}</td>
                <td class="col-num">${liq.size ? fmt(liq.size, 2) : '—'}</td>
                <td class="col-num">${liq.price ? fmt(liq.price, 4) : '—'}</td>
                <td class="col-num green">${liq.recovered ? fmtMNT(liq.recovered) : '—'}</td>
                <td class="col-num ${impactClass}">${liq.impact ? fmtMNT(liq.impact) : '—'}</td>
            </tr>`;
        }).join('');
    }

    function renderInsuranceChart(historyData) {
        // Simple canvas-based chart for fund history
        const canvas = D.insChart;
        ...
        const values = historyData.map(d => d.balance);
        ...
        historyData.forEach((point, i) => {
            ...
            const y = padding + (1 - (point.balance - minVal) / range) * chartHeight;
    ...
    async function executeOrder() {
        ...
        try {
            ...
            if (data && (data.success || data.order_id)) {
                const priceStr = isLimit ? ` @ ${fmt(price, 2)}` : ' @ Market';
                toast(`${side.toUpperCase()} ${qty} ${S.selected}${priceStr}`, side === 'buy' ? 'success' : 'sell');
                Sound.orderPlaced();
    ...
    function bindEvents() {
        ...
        $$('.btab').forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                // Fetch insurance data when insurance tab opened
                if (tab.dataset.tab === 'insurance') {
                    fetchInsuranceFund();
                }
        ...
        // Column resize handles (left ↔ center, center ↔ right)
        (function() {
            ...
            const MIN_LEFT = 180, MIN_CENTER = 300, MIN_RIGHT = 250;
            // Restore saved widths
            const savedLeft = parseInt(localStorage.getItem('cre_leftW'));
            const savedRight = parseInt(localStorage.getItem('cre_rightW'));
            if (savedLeft > 0 || savedRight > 0) {
                const lw = savedLeft || 260, rw = savedRight || 280;
                ws.style.gridTemplateColumns = `${lw}px 4px 1fr 4px ${rw}px`;
                left.style.minWidth = lw + 'px';
                left.style.maxWidth = lw + 'px';
                right.style.minWidth = rw + 'px';
            }
            ...
            function setupResize(handle, side) {
                ...
                handle.addEventListener('mousedown', function(e) {
                    ...
                    dragging = true;
                    startX = e.clientX;
                    startW = side === 'left' ? left.offsetWidth : right.offsetWidth;
                    handle.classList.add('dragging');
                ...
                document.addEventListener('mousemove', function(e) {
                    ...
                    let lw = side === 'left' ? Math.max(MIN_LEFT, startW + delta) : left.offsetWidth;
                    let rw = side === 'right' ? Math.max(MIN_RIGHT, startW - delta) : right.offsetWidth;
                    const avail = ws.offsetWidth - 8; // 2 handles × 4px
                    if (avail - lw - rw < MIN_CENTER) return;
                    ws.style.gridTemplateColumns = `${lw}px 4px 1fr 4px ${rw}px`;
                    left.style.minWidth = lw + 'px';
                    left.style.maxWidth = lw + 'px';
                    right.style.minWidth = rw + 'px';
                ...
                document.addEventListener('mouseup', function() {
                    ...
                    document.body.style.cursor = '';
                    document.body.style.userSelect = '';
                    localStorage.setItem('cre_leftW', left.offsetWidth);
                    localStorage.setItem('cre_rightW', right.offsetWidth);
                    if (window.chart) window.chart.applyOptions({});
                });
            }
        ...
        // Insurance fund refresh button
        if (D.insRefreshBtn) {
            D.insRefreshBtn.addEventListener('click', fetchInsuranceFund);
        }

        // Insurance timeframe buttons
        $$('.ins-tf-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                $$('.ins-tf-btn').forEach(b => b.classList.toggle('active', b === btn));
                // In real implementation, would fetch data for the selected timeframe
                fetchInsuranceFund();
            });
        });
    ...
    function openAlertsPopup() {
        if (!S.token) { toast('Login required to manage alerts', 'warn'); return; }
        const syms = (S.markets || []).map(m => m.symbol);
        const symOpts = syms.map(s => `<option value="${escHtml(s)}"${s === S.selected ? ' selected' : ''}>${escHtml(s)}</option>`).join('');
        const content = `
            <div style="padding:4px 0">
                <div style="display:flex;gap:4px;margin-bottom:8px;align-items:center;flex-wrap:wrap">
                    <select id="alertSym" class="t-input" style="padding:3px 6px;font-size:11px;width:140px">${symOpts}</select>
                    <input type="number" id="alertPrice" class="t-input" style="flex:1;padding:3px 6px;font-size:11px;min-width:80px" placeholder="Price ₮">
                    <select id="alertDir" class="t-input" style="padding:3px 6px;font-size:11px;width:80px">
                        <option value="ABOVE">Above ▲</option><option value="BELOW">Below ▼</option>
                    </select>
                    <button id="alertAddBtn" style="padding:3px 12px;background:var(--gold);color:var(--bg-0);border:none;font-size:11px;font-weight:600;cursor:pointer;border-radius:3px">Add</button>
                </div>
                <table class="popup-grid">
                    <thead><tr><th>Symbol</th><th class="num">Price</th><th>Direction</th><th>Action</th></tr></thead>
                    <tbody id="alertsBody"><tr><td colspan="4" style="text-align:center;color:var(--text-3);padding:16px">Loading...</td></tr></tbody>
                </table>
            </div>`;
        const pop = createPopup('alerts', '🔔 Price Alerts', 460, 320, content);

        async function loadAlerts() {
            const body = document.getElementById('alertsBody');
            if (!body) return;
            const data = await apiFetch('/alerts');
            if (!data || !Array.isArray(data.alerts || data)) {
                body.innerHTML = '<tr><td colspan="4" style="text-align:center;color:var(--text-3);padding:16px">No alerts set</td></tr>';
                return;
            }
            ...
            body.innerHTML = alerts.map(a => `<tr>
                ...
                <td class="num">${fmtMNT(a.price)}</td>
                <td>${a.direction === 'ABOVE' ? '▲ Above' : '▼ Below'}</td>
                <td><button class="alert-del" data-id="${a.id}" style="padding:2px 8px;background:var(--red);color:#fff;border:none;font-size:10px;cursor:pointer;border-radius:2px">✕</button></td>
            </tr>`).join('');
            body.querySelectorAll('.alert-del').forEach(btn => {
                btn.addEventListener('click', async () => {
                    await apiFetch('/alerts/' + btn.dataset.id, { method: 'DELETE' });
                    toast('Alert removed', 'info');
                    loadAlerts();
                });
            });
        }
        // Refresh data on every open (handles re-opens of existing popup)
        loadAlerts();

        // Wire add button only on first creation (event listeners persist)
        if (!pop._alertsWired) {
            pop._alertsWired = true;
            ...
            if (addBtn) addBtn.addEventListener('click', async () => {
                ...
                if (!sym || !price || price <= 0) { toast('Enter valid symbol and price', 'warn'); return; }
                const res = await apiFetch('/alerts', {
                    method: 'POST',
                    body: JSON.stringify({ symbol: sym, price, direction: dir })
                });
                if (res) {
                    toast(`Alert set: ${sym} ${dir === 'ABOVE' ? '▲' : '▼'} ${fmtMNT(price)}`, 'success');
    ...
    async function saveWorkspace() {
        if (!S.token) { toast('Login required to save workspaces', 'warn'); return; }
        ...
        const layout = {
            selected: S.selected,
            panels: {
                left: !ws?.classList.contains('hide-left'),
                book: !ws?.classList.contains('hide-book'),
                chart: !ws?.classList.contains('hide-chart'),
                trade: !ws?.classList.contains('hide-trade'),
                bottom: !ws?.classList.contains('hide-bottom')
            }
        };
        const res = await apiFetch('/workspaces', {
            method: 'POST',
            body: JSON.stringify({ name: name.trim(), layout: JSON.stringify(layout), selected_symbol: S.selected })
        });
        if (res) toast(`Workspace "${name.trim()}" saved`, 'success');
        else toast('Failed to save workspace', 'error');
    }

    async function openLoadWorkspacePopup() {
        if (!S.token) { toast('Login required to load workspaces', 'warn'); return; }
        const content = `
            <div style="padding:4px 0">
                <table class="popup-grid">
                    <thead><tr><th>Name</th><th>Symbol</th><th>Saved</th><th>Actions</th></tr></thead>
                    <tbody id="wsBody"><tr><td colspan="4" style="text-align:center;color:var(--text-3);padding:16px">Loading...</td></tr></tbody>
                </table>
            </div>`;
        const pop = createPopup('workspaces', '📂 Load Workspace', 440, 280, content);

        async function loadList() {
            const body = document.getElementById('wsBody');
            if (!body) return;
            const data = await apiFetch('/workspaces');
            if (!data || !Array.isArray(data.workspaces || data)) {
                body.innerHTML = '<tr><td colspan="4" style="text-align:center;color:var(--text-3);padding:16px">No saved workspaces</td></tr>';
                return;
            }
            ...
            body.innerHTML = list.map(w => {
                const ts = w.created_at ? new Date(w.created_at).toLocaleDateString() : '—';
                return `<tr>
                    <td style="color:var(--gold);cursor:pointer" class="ws-load" data-layout='${escHtml(w.layout || '{}')}'
                        data-sym="${escHtml(w.selected_symbol || '')}">${escHtml(w.name)}</td>
                    <td>${escHtml(w.selected_symbol || '—')}</td>
                    <td>${ts}</td>
                    <td><button class="ws-del" data-id="${w.id}" style="padding:2px 8px;background:var(--red);color:#fff;border:none;font-size:10px;cursor:pointer;border-radius:2px">✕</button></td>
                </tr>`;
            }).join('');
            body.querySelectorAll('.ws-load').forEach(el => {
                el.addEventListener('click', () => {
                    try {
                        const layout = JSON.parse(el.dataset.layout);
                        const ws = document.querySelector('.workspace');
                        if (ws && layout.panels) {
                            ws.classList.toggle('hide-left', !layout.panels.left);
                            ws.classList.toggle('hide-book', !layout.panels.book);
                            ws.classList.toggle('hide-chart', !layout.panels.chart);
                            ws.classList.toggle('hide-trade', !layout.panels.trade);
                            ws.classList.toggle('hide-bottom', !layout.panels.bottom);
                        }
                        if (el.dataset.sym) selectMarket(el.dataset.sym);
                        toast(`Workspace loaded`, 'success');
                        pop.el.style.display = 'none';
                    } catch (e) { toast('Failed to load layout', 'error'); }
                });
            });
            body.querySelectorAll('.ws-del').forEach(btn => {
                btn.addEventListener('click', async () => {
                    await apiFetch('/workspaces/' + btn.dataset.id, { method: 'DELETE' });
    ...
    async function openAdminPopup() {
        if (!S.token) { toast('Login required', 'warn'); return; }
        const content = `
            <div style="padding:8px 0">
                <div id="adminMetrics" style="font-size:11px;color:var(--text-2)">Loading dashboard...</div>
            </div>`;
        const pop = createPopup('admin', '⚙️ Admin Dashboard', 520, 400, content);

        const metricsEl = document.getElementById('adminMetrics');
        if (!metricsEl) return;
        const data = await apiFetch('/admin/dashboard');
        if (!data) { metricsEl.textContent = 'Access denied or unavailable'; return; }

        function fmtUptime(s) {
            const d = Math.floor(s / 86400), h = Math.floor((s % 86400) / 3600), m = Math.floor((s % 3600) / 60);
            return `${d}d ${h}h ${m}m`;
        }
        const halted = (data.products_halted || []).map(p => `<span style="color:var(--red)">${escHtml(p.symbol)} (${p.state})</span>`).join(', ') || '<span style="color:var(--green)">None</span>';
        const topProds = (data.top_products_by_volume || []).map(p =>
            `<tr><td>${escHtml(p.symbol)}</td><td class="num">${fmtMNT(p.volume)}</td></tr>`
        ).join('') || '<tr><td colspan="2" style="color:var(--text-3)">No data</td></tr>';

        metricsEl.innerHTML = `
            <table class="popup-grid" style="margin-bottom:12px">
                <thead><tr><th>Metric</th><th class="num">Value</th></tr></thead>
                <tbody>
                    <tr><td>Server Uptime</td><td class="num">${fmtUptime(data.server_uptime_seconds || 0)}</td></tr>
                    <tr><td>Total Users</td><td class="num">${data.total_users || 0}</td></tr>
                    <tr><td>Active Users (24h)</td><td class="num">${data.active_users_24h || 0}</td></tr>
                    <tr><td>24h Volume</td><td class="num">${fmtMNT(data.total_volume_24h_mnt)}</td></tr>
                    <tr><td>Open Positions</td><td class="num">${data.total_open_positions || 0}</td></tr>
                    <tr><td>Open Orders</td><td class="num">${data.total_open_orders || 0}</td></tr>
                    <tr><td>Insurance Fund</td><td class="num gold">${fmtMNT(data.insurance_fund_balance)}</td></tr>
                    <tr><td>Halted Products</td><td class="num">${halted}</td></tr>
                    <tr><td>Total Trades (all time)</td><td class="num">${data.total_trades_all_time || 0}</td></tr>
                    <tr><td>Total Volume (all time)</td><td class="num">${fmtMNT(data.total_volume_all_time_mnt)}</td></tr>
                </tbody>
            </table>
            <div style="font-weight:600;font-size:11px;margin-bottom:4px;color:var(--text-1)">Top Products by 24h Volume</div>
            <table class="popup-grid">
                <thead><tr><th>Product</th><th class="num">Volume ₮</th></tr></thead>
    ...
    function initMenuBar() {
        ...
        act('menuSaveWorkspace', () => saveWorkspace());
        act('menuLoadWorkspace', () => openLoadWorkspacePopup());
